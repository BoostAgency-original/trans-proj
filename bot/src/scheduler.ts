    ...
    for (const user of users) {
      ...
      if (userTime === eveningTime) {
        ...
        if (daysSinceIntro > 4 && !isActive) {
           ...
           // Отправляем сообщение о необходимости подписки ТОЛЬКО если это время утренней рассылки
           // Или если мы хотим напомнить об этом. Но лучше не спамить.
           // В текущей логике мы просто пропускаем отправку контента.
           // Пользователь просит сообщение "It's time to buy a subscription" с кнопками.
...
Открой практику полностью — и начни управлять реальностью осознанно.`;
           try {
             await bot.api.sendMessage(user.telegramId.toString(), subMsg, {
                reply_markup: getSubscriptionKeyboard() // Добавляем кнопки тарифов
             });
           } catch (e) {
             console.error(`Failed to send inactive sub msg to ${user.id}`, e);
           }
           continue;
        }
