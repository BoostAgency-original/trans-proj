...
/**
 * Сид только для bot_messages:
 * - добавляет недостающие ключи
 * - НЕ затирает тексты, которые уже были изменены в админке
 */
const MESSAGES: BotMessageSeed[] = [
  ...
  },
  {
    ...
    text:
      ...
      'Подарок одноразовый.',
    description: 'Текст для кнопки “Отправить другу” (plain text). Плейсхолдеры: {duration} {days} {link} {start_cmd}',
  },
  {
    key: 'unknown_message',
    category: 'general',
...
async function main() {
  console.log('🌱 Seed messages: start');

  for (const msg of MESSAGES) {
    await prisma.botMessage.upsert({
      where: { key: msg.key },
      // Важно: НЕ трогаем text, если ключ уже есть (админка могла менять текст)
      update: {
  ...
  console.log(`✅ Seed messages: done (${MESSAGES.length} keys ensured)`);
}

main()
  .catch((e) => {
    console.error('❌ Seed messages error:', e);
    process.exit(1);
  })
  .finally(async () => {
